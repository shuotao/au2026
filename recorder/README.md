...
**設計重點：錄影不依賴自動化成功。** 播放鍵找不到、瀏覽器自動化壞掉，
程式仍然照時間錄下整個畫面 —— 因為 OBS 錄的是整個螢幕，不是瀏覽器內部。
...
| 模式 | 怎麼運作 | 適用 |
|---|---|---|
| `launch`（預設） | 程式自己開一個它控制得到的瀏覽器，登入狀態存在 `browser-profile/`。第一次跑時你在那個視窗登入 AU2026、把視窗拖到要錄的螢幕；之後它每次都開在那裡，程式導同一個視窗。 | 一般情況 |
